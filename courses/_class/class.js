...
/* 발표를 마친 표시.
   예전에는 이 브라우저에만 남겼다. 기기를 바꾸면 사라지고, 강의실 컴퓨터와
   연구실 컴퓨터가 서로 몰랐다. 누가 언제 발표했는지도 남지 않아 학기말에
   참여도를 따질 근거가 없었다. 이제 서버에 둔다. 교수만 읽고 쓴다.
   브라우저에 있던 옛 표시는 처음 들어올 때 한 번 서버로 밀어 올린다. */
const SPOKE = C.id + "-spoken";       // 옛 자리. 옮기고 나면 비운다.
const SPOKEN = C.id + "_spoken";      // 새 자리. 서버.
let stopSpoken = null;
let movedSpoken = false;

function watchSpoken() {
  if (!state.me?.prof) return;        // 학생은 규칙이 막는다. 붙이지 않는다.
...
/* 표시를 켜고 끈다. 켤 때 누가 언제 했는지 함께 남긴다. */
 ...
 * 부족하다. 숨은 것도 키는 차지하여 뒤에 빈 종이가 따라 나온다.
 *
